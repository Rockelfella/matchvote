...
DESCRIPTION_DE = "Choreografie (Fankultur)"
DESCRIPTION_EN = "choreography (fan culture)"
LEGACY_MARKER = "auto_other_choreo"
    ...
    if use_legacy:
        sql = text("""
            ...
            where match_id = cast(:match_id as uuid)
              and scene_type = 'OTHER'
              and minute = 0
              and description = :marker
            limit 1
        """)
        params = {"match_id": str(match_id), "marker": LEGACY_MARKER}
    else:
        sql = text("""
            select 1
            from referee_ratings.scenes
            ...
            limit 1
        """)
        params = {"match_id": str(match_id), "description_de": DESCRIPTION_DE, "description_en": DESCRIPTION_EN}
...
def insert_scene(conn, match_id, use_legacy):
    if use_legacy:
        sql = text("""
            ...
            values
              (:match_id, 0, null, 'OTHER', :description, :description_de, :description_en, :is_released, :release_time, :created_by)
        """)
    else:
        sql = text("""
            insert into referee_ratings.scenes
            ...
            values
              (:match_id, 0, null, 'OTHER', :description_de, :description_en, :is_released, :release_time, :created_by)
        """)
    params = {
...
def main():
    parser = argparse.ArgumentParser(description="Seed OTHER scenes (Choreografie) at minute 0 for each match")
    ...
    with engine.begin() as conn:
        ...
        rows = conn.execute(text("select match_id from referee_ratings.matches")).mappings().all()
        for row in rows:
            total += 1
            ...
            if scene_exists(conn, match_id, use_legacy):
                ...
                continue
            if args.dry_run:
                log(f"[dry-run] Would insert OTHER scene match_id={match_id}")
                inserted += 1
                continue
